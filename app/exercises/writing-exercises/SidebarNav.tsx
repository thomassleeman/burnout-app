...
const SidebarNav: React.FC<SidebarNavProps> = ({ sections }) => {
  ...
  console.log("section: ", sections[1].prompts);
          ...
          {sections.map((section) => (
            <li key={section.id} className="space-y-2">
              <div className="flex items-center space-x-2 text-sm">
                <ChevronRightIcon className="h-5 w-5 text-sky-700 dark:text-sky-300" />
                <Link
                  href={`#${section.id}`}
                  className={`text-sky-700 hover:underline dark:text-sky-300 ${
                    pathname.includes(section.id) ? "text-sky-800" : ""
                  }`}
                >
                  {section.title}
                </Link>
...
//   return (
//     <aside className="sticky top-0 hidden overflow-y-auto p-4 py-10 dark:border-slate-800 lg:block">
//       <nav className="space-y-4">
//         <h2 className="text-sm font-semibold">On This Page</h2>
//         <ul className="space-y-4">
//           {sections.map((section) => (
